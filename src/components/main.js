  ...
  render() {
    return (
      <div className="layout">
        <h1 className="story">{data.filter((item) => item.id === this.state.destinationId)[0].story}</h1>
        <Options
          nextArch={this.nextArch}
          optionA={data.filter((item) => item.id === this.state.destinationId)[0].options.a}
          optionB={data.filter((item) => item.id === this.state.destinationId)[0].options.b}
        />
        <Reminder
